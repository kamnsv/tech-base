...
> Если в таблице составной ключ, то часть ключа, не должен быть потенциальным кулючем, других частей ключа.
...
* Работа с БД в стиле ООП.
* Интерфейс для `CRUD`.

> CRUD.(создание (англ. create), чтение (read), модификация (update), удаление (delete).)

* Избаление от SQL-кода.
* Устраниение семантического разрыва.

✅ Меньше ошибок в SQL-запросах.
✅ Более быстрая разработка.

⛔ Не всегда оптимальбные SQL-запросы.
...
**Microsoft SQL Server** — система управления реляционными базами данных (РСУБД), разработанная корпорацией Microsoft. Основной используемый язык запросов — Transact-SQL. **Transact-SQL** (T-SQL) — процедурное расширение языка SQL:
