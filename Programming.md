...
> класс Строитель отдает конструктор Директору(ам), они строют разные представления объекта.
...
> позволяет копировать объекты, не вдаваясь в подробности их реализации. У класса есть метод copy(), который копирут сокрытые данные тоже.
	...
	6.  Одиночка (Singleton, monostate) > гарантирует, что у класса есть только один экземпляр, и предоставляет к нему глобальную точку доступа.
